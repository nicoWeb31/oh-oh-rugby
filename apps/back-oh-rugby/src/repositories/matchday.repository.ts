...
export async function getMatchdayById(
  id: string,
): Promise<Matchday | undefined> {
  const result = await ddb.send(
    new GetCommand({ TableName: TABLE_NAME, Key: matchdayKey(id) }),
  );
...
export async function getMatchdaysByIds(ids: string[]): Promise<Matchday[]> {
  ...
  for (let i = 0; i < ids.length; i += 100) {
    ...
    const result = await ddb.send(
      new BatchGetCommand({
        RequestItems: { [TABLE_NAME]: { Keys: chunk.map(matchdayKey) } },
      }),
    );
  ...
  return ids
    .map((id) => byId.get(id))
    .filter((matchday): matchday is Matchday => Boolean(matchday));
}
